...
def problem_three(request):
  instructor_two = Instructor.objects.get (pk=2)
  course_by_2 = Course.objects.filter (instructor__course=instructor_two)
  for course in course_by_2:
    print (
      f'Instructor Name: {instructor_two.first_name} {instructor_two.last_name}\n Courses: \n {course_by_2.name}'
    )
...
def problem_four(request):
  ...
  print(
    f'Student Count: {number_of_students}\n Course Count: {number_of_courses}\n Instructor Count: {number_of_instructors}'
  )
  return complete(request)
...
def problem_five(request):
  new_student = Student.objects.create (first_name='Bryan', last_name= 'Davis', year=2023, gpa=3.2)
  new_student_id = Student.objects.get(pk=12)
...
def problem_six(request):
    ...
    student_gpa_update = Student.objects.filter(pk=12).update(gpa = 3.0)
    student_id = Student.objects.get(pk=12)
    print (
      f'Id: {student_id.pk} \nFull Name: {student_id.first_name} {student_id.last_name} \n GPA: {student_id.gpa}'
    )
...
def problem_seven(request):
    del_student = Student.objects.filter (pk=11).delete()
